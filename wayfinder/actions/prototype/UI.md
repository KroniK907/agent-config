...
Pull scope from the task **What to build** when present.
...
Once a variant has won (or a synthesis is named):
...
Do not merge prototype switcher or losing variants into production in this task unless **Done when** explicitly requires it.

## Anti-patterns

- **Variants that differ only in colour or copy.** That's a tweak, not a prototype. Real variants disagree about structure.
- **Sharing too much code between variants.** A shared `Button` is fine; a shared `PageLayout` defeats the point. Each variant should be free to throw out the layout.
