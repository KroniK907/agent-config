...
T1=$(create_ticket "Define decision-log handoff to write-a-prd" "$DIR/todo-decision-log-prd.md" "wayfinder:todo,wayfinder:grilling,wayfinder:hitl")
T1N=$(echo "$T1" | rg -o '[0-9]+$')
T2=$(create_ticket "Update grill-me to append scoped GM-xx to map decision log" "$DIR/todo-grill-me-integration.md" "wayfinder:todo,wayfinder:task,wayfinder:hitl")
T2N=$(echo "$T2" | rg -o '[0-9]+$')
T3=$(create_ticket "Specify research ticket workflow (skill or Task pattern)" "$DIR/todo-research-workflow.md" "wayfinder:todo,wayfinder:grilling,wayfinder:hitl")
T3N=$(echo "$T3" | rg -o '[0-9]+$')
T4=$(create_ticket "GitHub tracker setup for skills repo maps" "$DIR/todo-github-tracker.md" "wayfinder:todo,wayfinder:task,wayfinder:afk")
T4N=$(echo "$T4" | rg -o '[0-9]+$')
T5=$(create_ticket "Subfeature map worked example in REFERENCE or EXAMPLES" "$DIR/todo-subfeature-example.md" "wayfinder:todo,wayfinder:prototype,wayfinder:hitl")
T5N=$(echo "$T5" | rg -o '[0-9]+$')
T6=$(create_ticket "Reconcile / post-implementation map update skill or mode" "$DIR/todo-reconcile.md" "wayfinder:todo,wayfinder:grilling,wayfinder:hitl")
T6N=$(echo "$T6" | rg -o '[0-9]+$')
T7=$(create_ticket "Cloud AFK automation contract for wayfinder:afk tickets" "$DIR/todo-afk-contract.md" "wayfinder:todo,wayfinder:grilling,wayfinder:hitl")
T7N=$(echo "$T7" | rg -o '[0-9]+$')

echo "Creating decision log..."
LOG=$(create_ticket "Wayfinder-Ecosystem:Decision-Log" "$DIR/decision-log.md" "wayfinder:decision-log")
...
MAP=$(gh issue create -R "$REPO" --title "Wayfinder-Ecosystem:Map" --label "wayfinder:map" --body-file /tmp/wayfinder-map-body.md)
