...
- **Subsection headings = h5 only** (see **Heading rule**).
...
1. **Finished summary** - deliver in chat inside a **single markdown fenced code block** so the user can copy it in one click. Before sending, scan for `##`, `###`, or `####` and replace with `#####`.
