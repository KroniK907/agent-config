...
Once sous-vide hands over the **Ask** table, work **depth-first**: settle one branch fully (or agree to defer it) before starting the next.
...
When every Ask id is answered, stop the grill format and return the answers to sous-vide for the next wave. Skip the session-complete question. Reconcile waits until sous-vide finishes.
...
**Follow Up:** The decision from the last Q/A in concrete terms (agreed, settled, or deferred, and why). If the user asked questions instead of answering, answer them here. First reply: restate the plan and give one-line reasons for any zones closed as N/A.
...
2. Stay on the same branch until it is settled, then move to a sibling or parent.
